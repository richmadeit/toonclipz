...
(()=>{const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;const button=document.querySelector('.motion-toggle');button.addEventListener('click',()=>{const paused=document.body.classList.toggle('motion-paused');button.setAttribute('aria-pressed',String(paused));button.textContent=paused?'Resume motion':'Pause motion';});if(reduced){button.hidden=true;return;}if(!('IntersectionObserver' in window))return;const io=new IntersectionObserver(entries=>entries.forEach(e=>{if(e.isIntersecting){e.target.classList.add('in-view');io.unobserve(e.target)}}),{threshold:.08});document.querySelectorAll('.intro,.buy-panel,.free-panel,.tota-grid,.comparison-grid,.cards,.pulse').forEach(el=>{el.classList.add('reveal-ready');io.observe(el)});const counter=new IntersectionObserver(entries=>entries.forEach(e=>{if(!e.isIntersecting)return;counter.unobserve(e.target);const el=e.target,end=Number(el.dataset.count),final=el.textContent,start=performance.now();function step(t){const p=Math.min((t-start)/1000,1),v=Math.round(end*(1-Math.pow(1-p,3)));el.textContent=p===1?final:el.dataset.compact?(v>=1000?(v/1000).toFixed(1)+'K':String(v)):String(v);if(p<1)requestAnimationFrame(step)}requestAnimationFrame(step)}),{threshold:.5});document.querySelectorAll('[data-count]').forEach(el=>counter.observe(el));})();