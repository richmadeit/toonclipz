...
export async function readPurchases(env,since,fetcher=fetch){
  const key=env.STRIPE_SECRET_KEY,allowed=(env.STRIPE_PAYMENT_LINK_IDS||'').split(',').map(x=>x.trim()).filter(Boolean);
  if(!key||!allowed.length)return {available:false,reason:'Live Stripe verification is not configured.'};
  ...
  for(let page=0;page<5;page++){
    ...
    url.searchParams.set('expand[]','data.payment_intent.latest_charge');if(after)url.searchParams.set('starting_after',after);
    ...
    for(const s of data.data){
      if(!s.livemode||s.mode!=='payment'||s.status!=='complete'||s.payment_status!=='paid'||!allowed.includes(s.payment_link)||s.currency!=='usd'||s.amount_subtotal!==2500)continue;
