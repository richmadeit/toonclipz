...
export function checkSession(session,id,linkId){
  ...
  if(session.id!==id||session.livemode!==true||session.mode!=='payment'||link!==linkId||
...
export function createHandler({env=process.env,requestStripe=fetch}={}){
  return async function handler(request){
    ...
    if(!key||!/^plink_[A-Za-z0-9]+$/.test(linkId||''))return reply(503,{status:'verification_unavailable'});
    try{
      ...
      const response=await requestStripe(url,{headers:{Authorization:'Bearer '+key},signal:AbortSignal.timeout(10000)});
