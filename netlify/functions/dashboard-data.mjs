...
export default async function handler(req){
  ...
  const cached=cache.get(range);if(cached&&now-cached.at<20000)return json(200,cached.data);
  try{
    const [traffic,purchases]=await Promise.all([readTraffic(getStore({name:'toonclipz-traffic-v1',consistency:'strong'}),since,now),readPurchases(process.env,since).catch(()=>({available:false,reason:'Stripe could not be reached. Purchases are unavailable, not zero.'}))]);
    const data={traffic,purchases,updatedAt:now,since};cache.set(range,{at:now,data});return json(200,data);
